...
class Downloader {
    constructor(url, method, header, body, json, proxy) {
        this.url = url;
        this.proxy = [];
        this.option = {};
    ...
    //获取随机代理
    getProxy() {
        let Range = this.proxy.length - 1;
        let ran = Math.random();
        let ranProxyIndex = Math.round(Range * ran);
        return this.proxy[ranProxyIndex];
    }
            ...
            return new Promise(function (resolved, rejected) {
                request({
                    ...
                    if (!error) {
                        ...
                        logger.error('代理请求发送失败：' + error.message);
