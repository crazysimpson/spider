...
class Scheduler {
    ...
    addUrlToCrawled(url) {
        this.redisClient.sadd('crawledUrl', url, (err, value) => {
            if (err) {
                logger.info('添加url到已读取集合失败');
                logger.error(err.message);
            } else {
                logger.info('添加url到已读取集合成功');
            }
    ...
    async addUncrawlUrls(urls) {
        let addList = [];
        urls.forEach((value) => {
            addList.push(this.addUrl(value));
        });
        return Promise.race(addList);
    }
    ...
    schedule() {
        ...
        if (this.scheduler != null) {
            logger.error('调度自定义scheduler');
            return this.scheduler();
        }

        logger.info('调度默认scheduler');
...
//var urls = ['http://www.baidu.com', 'http://crazysimpson.cn'];
